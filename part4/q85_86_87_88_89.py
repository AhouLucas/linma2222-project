...
############ Definition of psi and greedy policy (based on psi) ############
...
def greedy_from_theta_plus_scaled(theta, psi, constrained=False):
    ...
    def policy(x):
        ...
        x_s = x / sx
        q, za, zu = float(x_s[0]), float(x_s[1]), float(x_s[2])
...
def greedy_policy_from_theta_grid(theta, psi, n_grid=201, constrained=False):
    us = np.linspace(-1, 1, n_grid)
    def policy(x):
        if constrained:
            q = float(x[0])
            us = np.linspace(-q, 1 - q, n_grid)
        else:
            us = np.linspace(-1, 1, n_grid)
        return float(
            us[np.argmax([theta @ psi(x, u) for u in us])]
        )
    return policy
...
def q8_7_d4():
    ...
    pi_lspepi, theta_Q_last, eta_hat_last, K_array, policies = lspe_pi(initial_policy=pol_cl,
        T_data=40, burn_in=20, N_traj=400, sigma_exp=0.2, 
        # psi=psi_d4, get_pol_fn=greedy_policy_from_theta,
        # psi=psi_plus, get_pol_fn=greedy_policy_from_theta,

        # psi=scale_psi(psi_plus), get_pol_fn=greedy_from_theta_plus_scaled,
        # psi=psi_plus, get_pol_fn=greedy_from_theta_plus,
        psi=scale_psi(psi_plus), get_pol_fn=greedy_policy_from_theta_grid,
